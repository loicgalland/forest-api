...
import { Bed, BedArray, File, Hosting } from "../database/models";
...
export const createHosting = async (
  ...
  body.equipments = JSON.parse(body.equipments).map(
    (equipmentId: string) => new mongoose.Types.ObjectId(equipmentId),
  );
    ...
    const beds = await Promise.all(
      input.beds.map(async (bed) => {
        ...
        if (!existingBed) {
          return res
            .status(404)
            .json({ error: `Bed with ID ${bedId} not found` });
        }
...
export const getAllHosting = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  ...
  try {
    if (fullAccess === "true") {
      ...
      hostings = await Hosting.find({ visible: true, isSpotlight: true })
        .populate({
          path: "beds",
          populate: {
            path: "bed",
...
export const getOneHosting = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const updateHosting = async (
  ...
  try {
    ...
    req.body.beds.forEach((bed) => {
      ...
      if (bedId && quantity) {
        const existingBedIndex = hosting.beds.findIndex((b) =>
          b.bed.equals(new mongoose.Types.ObjectId(bedId)),
        );
    ...
    if (imageToDelete) {
      hosting.images = hosting.images.filter(
        (imageId) => !imageToDelete.includes(imageId.toString()),
      );
    }
    ...
    if (hosting.beds && hosting.beds.length)
      hosting.capacity = await calculateCapacity(hosting.beds);
